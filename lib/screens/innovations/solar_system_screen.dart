...
const _planets = <_PlanetData>[
  _PlanetData(
    name: 'Mercury', color: Color(0xFFB0ACA6), colorDark: Color(0xFF5A5652),
    visualRadius: 4.0, orbitFraction: 0.092, periodSeconds: 4.8, startAngle: 0.7,
  ),
  _PlanetData(
    name: 'Venus', color: Color(0xFFE8C876), colorDark: Color(0xFF8A6F32),
    visualRadius: 6.2, orbitFraction: 0.138, periodSeconds: 7.8, startAngle: 2.4,
    atmosphere: Color(0xFFFFF3C4),
  ),
  _PlanetData(
    name: 'Earth', color: Color(0xFF4DA6FF), colorDark: Color(0xFF1A4070),
    visualRadius: 6.8, orbitFraction: 0.195, periodSeconds: 12.0, startAngle: 4.9,
    atmosphere: Color(0xFF80D0FF),
  ),
  _PlanetData(
    name: 'Mars', color: Color(0xFFE06040), colorDark: Color(0xFF6D2A1A),
    visualRadius: 5.0, orbitFraction: 0.255, periodSeconds: 20.0, startAngle: 1.1,
    atmosphere: Color(0xFFFF9070),
  ),
  _PlanetData(
    name: 'Jupiter', color: Color(0xFFD4A96A), colorDark: Color(0xFF6A5030),
    visualRadius: 14.0, orbitFraction: 0.430, periodSeconds: 36.0, startAngle: 3.5,
    bands: [
      _Band(Color(0xFFC8906A), -0.35, 0.20),
      _Band(Color(0xFFE8C890), 0.0, 0.15),
      _Band(Color(0xFFB07848), 0.30, 0.22),
      _Band(Color(0xFFDEB880), -0.60, 0.12),
    ],
  ),
  _PlanetData(
    name: 'Saturn', color: Color(0xFFEAD5A0), colorDark: Color(0xFF806838),
    visualRadius: 11.5, orbitFraction: 0.575, periodSeconds: 58.0, startAngle: 5.6,
    hasRings: true,
  ),
  _PlanetData(
    name: 'Uranus', color: Color(0xFF80DEEA), colorDark: Color(0xFF2A6070),
    visualRadius: 8.5, orbitFraction: 0.760, periodSeconds: 82.0, startAngle: 0.2,
    atmosphere: Color(0xFFA0F0F8),
  ),
  _PlanetData(
    name: 'Neptune', color: Color(0xFF5C6BC0), colorDark: Color(0xFF283060),
    visualRadius: 7.8, orbitFraction: 0.950, periodSeconds: 118.0, startAngle: 2.8,
...
class _PlanetPos {
  ...
  const _PlanetPos(
      this.index, this.screenPos, this.z, this.angle, this.scale, this.brightness);
}
...
class _SolarSystemPainter extends CustomPainter {
  ...
  void _drawBackground(Canvas canvas, Size size, Offset center, double maxOrbit) {
    canvas.drawRect(
      Offset.zero & size,
      Paint()
    ...
    canvas.drawCircle(center, maxOrbit * 0.45, Paint()
      ..color = const Color(0xFFFF8F00).withOpacity(0.020)
  ...
  void _drawOrbitalPlaneGrid(Canvas canvas, Offset center, double maxOrbit) {
    ...
    for (int i = 1; i <= 14; i++) {
      ...
      canvas.drawOval(Rect.fromCenter(
        center: center, width: r * 2, height: r * 2 * _tiltFactor), gridPaint);
    }
    ...
    for (int i = 0; i < 12; i++) {
      ...
      canvas.drawLine(center, Offset(
        ...
        center.dy + far * math.sin(angle) * _tiltFactor), radialPaint);
    }
  }
  ...
  void _drawOrbits(Canvas canvas, Offset center, double maxOrbit) {
    for (final p in _planets) {
      ...
      final rect = Rect.fromCenter(
        center: center, width: r * 2, height: r * 2 * _tiltFactor);
  ...
  void _drawOrbitTrail(Canvas canvas, Offset center, _PlanetData p,
      _PlanetPos pp, double maxOrbit) {
    final r = p.orbitFraction * maxOrbit;
    const sweep = 0.50;
    final rect = Rect.fromCenter(
      center: center, width: r * 2, height: r * 2 * _tiltFactor);
    canvas.drawArc(rect, pp.angle - sweep, sweep, false, Paint()
      ..color = p.color.withOpacity(0.10 * pp.brightness.clamp(0.5, 1.5))
      ..style = PaintingStyle.stroke
      ..strokeWidth = 3.0 * pp.scale
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3));
    canvas.drawArc(rect, pp.angle - sweep * 0.5, sweep * 0.5, false, Paint()
      ..color = p.color.withOpacity(0.20 * pp.brightness.clamp(0.5, 1.5))
      ..style = PaintingStyle.stroke
  ...
  void _drawSun(Canvas canvas, Offset c, double r) {
    ...
    canvas.drawCircle(c, r * 6.0 * pulse2, Paint()
      ..color = const Color(0xFFFF6D00).withOpacity(0.010)
    ...
    for (int i = 0; i < 8; i++) {
      ...
      canvas.drawLine(
        ...
        Paint()
          ..color = const Color(0xFFFFCC02).withOpacity(0.04)
          ..strokeWidth = 2.5
          ..strokeCap = StrokeCap.round
    ...
    for (int i = 6; i >= 1; i--) {
      canvas.drawCircle(c, r * (1.0 + i * 0.45) * pulse, Paint()
        ..color = const Color(0xFFFF9800).withOpacity(0.022 * i)
        ..maskFilter = MaskFilter.blur(BlurStyle.normal, 10.0 + i * 5));
    }

    canvas.drawCircle(c, r * 1.5, Paint()
      ..color = const Color(0xFFFFCC02).withOpacity(0.24)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8));

    canvas.drawCircle(c, r, Paint()
      ..shader = ui.Gradient.radial(c, r, const [
        Color(0xFFFFFFEE), Color(0xFFFFF176), Color(0xFFFFCA28),
    ...
    canvas.drawCircle(
      Offset(c.dx + r * 0.35 * math.cos(spotA),
             c.dy + r * 0.18 * math.sin(spotA)),
      r * 0.11, Paint()..color = const Color(0xFFCC7700).withOpacity(0.30));
    canvas.drawCircle(
      Offset(c.dx + r * 0.2 * math.cos(spotA + 2.2),
             c.dy + r * 0.30 * math.sin(spotA + 1.0)),
      r * 0.07, Paint()..color = const Color(0xFFCC7700).withOpacity(0.22));
    canvas.restore();
  }

  void _drawFullPlanet(Canvas canvas, _PlanetPos pp, Offset sunCenter, double sunR) {
    ...
    if (p.bands != null) {
      ...
      for (final band in p.bands!) {
        ...
        canvas.drawRect(
          ...
          Paint()..color = band.color.withOpacity(0.50 * bright));
      }
      canvas.restore();
    }

    canvas.drawCircle(pos + ld * r * 0.28, r * 0.36, Paint()
      ..shader = ui.Gradient.radial(
        pos + ld * r * 0.28, r * 0.36,
        [Colors.white.withOpacity(0.42 * bright), Colors.white.withOpacity(0.0)]));
  ...
  void _drawRings(Canvas canvas, Offset pos, double r, bool front) {
    void arc(double th, Color c, double mul) {
      final rect = Rect.fromCenter(
        center: pos, width: r * 2.6 * mul * 2, height: r * 0.55 * mul * 2);
      canvas.drawArc(rect, front ? 0 : math.pi, math.pi, false, Paint()
        ..color = c ..style = PaintingStyle.stroke ..strokeWidth = th);
    }
    arc(4.5, const Color(0xFFD4B896).withOpacity(0.55), 1.0);
    arc(1.5, Colors.transparent, 0.90);
    arc(2.5, const Color(0xFFBDA882).withOpacity(0.38), 0.83);
    arc(1.0, const Color(0xFFE8D0B0).withOpacity(0.15), 1.20);
  }
  ...
  void _drawMoon(Canvas canvas, Offset center, double maxOrbit) {
    ...
    canvas.drawOval(Rect.fromCenter(
      ...
      height: moonOrbit * 2 * _tiltFactor), Paint()
      ..color = Colors.white.withOpacity(0.06)
      ..style = PaintingStyle.stroke ..strokeWidth = 0.3);

    canvas.drawCircle(moonPos, 4.5, Paint()
    ...
    final dist = toSun.distance.clamp(1.0, double.infinity);
    final ld = Offset(toSun.dx / dist, toSun.dy / dist);
    canvas.drawCircle(moonPos, 2.4, Paint()
      ..shader = ui.Gradient.linear(moonPos + ld * 2.4, moonPos - ld * 2.4,
        const [Color(0xFFE0E4E8), Color(0xFF555E5E)]));
  }
  ...
  void _drawAsteroidBelt(Canvas canvas, Offset center, double maxOrbit) {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      canvas.drawCircle(Offset(ax, ay), sz * (1.0 + zNorm * 0.15), Paint()
        ..color = Colors.white.withOpacity(
            (rng.nextDouble() * 0.08 + 0.02) * depthOp));
    }
  }

  void _drawLabel(Canvas canvas, Offset pos, String name, double r,
    ...
    canvas.drawRect(
      Rect.fromLTWH(lx - 3, ly - 1, tp.width + 6, tp.height + 2), Paint()
        ..color = color.withOpacity(0.10 * bright)
