...
class _RituChakraState extends State<RituChakraGameScreen> with TickerProviderStateMixin {
  // Game State
  ...
  late AnimationController _sunCtrl, _seasonCtrl, _pulseCtrl, _weatherCtrl, _glowCtrl;
  late AnimationController _floatCtrl, _rotateCtrl, _shimmerCtrl, _breatheCtrl;
  late Animation<double> _sunAnim, _seasonAnim, _pulseAnim, _weatherAnim, _glowAnim;
  ...
  void _initAnimations() {
    ...
    _sunCtrl = AnimationController(vsync: this, duration: const Duration(seconds: 60))..repeat();
    _sunAnim = Tween<double>(begin: 0, end: 2 * math.pi).animate(_sunCtrl);
    
    // Season transition (20 seconds)
    _seasonCtrl = AnimationController(vsync: this, duration: const Duration(seconds: 20))..repeat();
    ...
    _weatherCtrl = AnimationController(vsync: this, duration: const Duration(seconds: 3))..repeat();
    ...
    _floatAnim = Tween<double>(begin: -1.0, end: 1.0).animate(CurvedAnimation(parent: _floatCtrl, curve: Curves.easeInOut));
    ...
    _breatheAnim = Tween<double>(begin: 0.98, end: 1.02).animate(CurvedAnimation(parent: _breatheCtrl, curve: Curves.easeInOut));
  }
  
  void _initFarm() {
    _farmGrid = List.generate(4, (y) => List.generate(5, (x) => _Plot(x: x, y: y)));
  }
  ...
  void _updateWeatherParticles() {
    ...
    if (_currentWeather == 'rain' || ritu.id == 'varsha') {
      if (_rand.nextDouble() < 0.3) {
        _weatherParticles.add(_WeatherParticle(
    ...
    _weatherParticles = _weatherParticles.where((p) {
      p.x += p.vx;
      p.y += p.vy;
      return p.y < 600;
    }).toList();
    ...
    if (_weatherParticles.length > 100) {
      _weatherParticles = _weatherParticles.sublist(_weatherParticles.length - 100);
    }
  }
  ...
  void _triggerEvent() {
    ...
    final events = [
      ...
      if (currentRitu.id == 'vasant')
        _Event('basant_panchami', '🌸', 'Basant Panchami', 'Saraswati blesses your knowledge! +10 wisdom', () { _wisdom += 10; }),
      if (currentRitu.id == 'grishma')
        _Event('heat_wave', '🔥', 'Heat Wave', 'Extreme heat! Water evaporates faster', () => _water = (_water - 15).clamp(0, 100)),
      if (currentRitu.id == 'varsha')
        _Event('monsoon_blessing', '🌧️', 'Monsoon Blessing', 'Abundant rains! Water reserves full', () => _water = 100),
      if (currentRitu.id == 'sharad')
        _Event('harvest_festival', '🎊', 'Harvest Festival', 'Celebrate abundance! +30 energy, +15 wisdom', () { _energy = (_energy + 30).clamp(0, 100); _wisdom += 15; }),
      if (currentRitu.id == 'hemant')
        _Event('dew_blessing', '💧', 'Morning Dew', 'Natural moisture nourishes crops', () => _processCropsBonus(1.2)),
      if (currentRitu.id == 'shishir')
        _Event('frost_warning', '❄️', 'Frost Warning', 'Protect your crops from frost!', () => _damageCrops(10)),
      
      // General events
      _Event('drought', '🏜️', 'Drought Warning', 'Water levels dropping fast!', () => _water = (_water - 20).clamp(0, 100)),
      _Event('pest', '🐛', 'Pest Attack', 'Pests are eating your crops!', () => _damageCrops(15)),
      _Event('festival', '🪔', 'Harvest Festival', 'Celebrate! +20 energy, +10 wisdom', () { _energy = (_energy + 20).clamp(0, 100); _wisdom += 10; }),
      _Event('rain', '🌧️', 'Heavy Rain', 'Unexpected rain! Water reserves full.', () => _water = 100),
      _Event('market', '🏪', 'Market Day', 'Good prices! +50% harvest value.', () => _harvest = (_harvest * 1.5).toInt()),
      _Event('blessing', '🙏', 'Divine Blessing', 'The gods favor your farm! All resources +20', () {
      ...
      _Event('compost_ready', '🌱', 'Compost Ready', 'Your compost is ready! +15 fertilizer', () => _fertilizer = (_fertilizer + 15).clamp(0, 100)),
      _Event('community_help', '🤝', 'Community Help', 'Neighbors help! +10 seeds, +10 energy', () {
  ...
  void _waterPlot(int x, int y) {
    ...
    setState(() {
      ...
      _water -= 5;
      _energy -= 1;
      _spawnParticles(x * 70.0 + 35, y * 70.0 + 35, Colors.blue);
    });
  }
  ...
  void _fertilizePlot(int x, int y) {
    ...
    setState(() {
      ...
      _fertilizer -= 5;
      _energy -= 2;
      _spawnParticles(x * 70.0 + 35, y * 70.0 + 35, Colors.brown);
    });
  }
  ...
  void _harvestPlot(int x, int y) {
    ...
    setState(() {
      ...
      _spawnParticles(x * 70.0 + 35, y * 70.0 + 35, Colors.amber, count: 20);
    });
  }
  ...
  void _buyResources(String type) {
    ...
    setState(() {
      _coins -= 10;
      switch (type) {
        case 'seeds': _seeds = (_seeds + 20).clamp(0, 100); break;
        case 'fertilizer': _fertilizer = (_fertilizer + 15).clamp(0, 100); break;
        case 'water': _water = (_water + 30).clamp(0, 100); break;
      }
    });
  }
      ...
      builder: (_, __) => AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [
              ritu.colorLight.withOpacity(isDark ? 0.3 : 0.6),
              ritu.colorDark.withOpacity(isDark ? 0.2 : 0.4),
  ...
  Widget _buildWeatherOverlay() {
    return IgnorePointer(
      child: Stack(
        children: _weatherParticles.map((p) {
          Widget particle;
          switch (p.type) {
            case 'rain':
              particle = Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    colors: [Colors.blue.withOpacity(0.1), Colors.blue.withOpacity(0.6)],
                  ),
            ...
            case 'snow':
              ...
              break;
            case 'petal':
              particle = Text('🌸', style: TextStyle(fontSize: 12, color: Colors.pink.withOpacity(0.8)));
              break;
            default:
  ...
  Widget _buildMenu(bool isDark) {
    final primaryColor = const Color(0xFFE65100); // Deep orange
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          // Clean season cards
          _buildCleanSeasonSection(isDark, primaryColor, textColor, subtleColor),
  ...
  Widget _buildCleanSeasonSection(bool isDark, Color primary, Color textColor, Color subtleColor) {
    return Column(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  color: isDark ? Colors.white.withOpacity(0.05) : ritu.colorLight.withOpacity(0.1),
                  borderRadius: BorderRadius.circular(16),
                  border: Border.all(
                    color: isDark ? Colors.white.withOpacity(0.1) : ritu.colorDark.withOpacity(0.2),
                  ),
                ),
                child: Column(
      ...
      builder: (_) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              _buildKnowledgeItem('🌸 Vasanta (Spring)', 'March-April: Perfect for sowing summer crops like wheat, barley, and pulses. Temperature: 20-30°C'),
              const SizedBox(height: 12),
              _buildKnowledgeItem('☀️ Grishma (Summer)', 'May-June: Heat-loving crops thrive. Time for cotton, sugarcane, and rice transplantation. Temperature: 30-40°C'),
              const SizedBox(height: 12),
              _buildKnowledgeItem('🌧️ Varsha (Monsoon)', 'July-August: Peak planting season. Heavy rainfall supports rice, maize, and vegetable cultivation. Temperature: 25-30°C'),
              const SizedBox(height: 12),
              _buildKnowledgeItem('🍂 Sharad (Autumn)', 'September-October: Harvest time for kharif crops. Sowing of rabi crops begins. Temperature: 20-25°C'),
              const SizedBox(height: 12),
              _buildKnowledgeItem('❄️ Hemanta (Winter)', 'November-December: Cool season crops. Wheat, mustard, and peas grow well. Temperature: 10-20°C'),
              const SizedBox(height: 12),
              _buildKnowledgeItem('🌱 Shishira (Late Winter)', 'January-February: Peak harvest season. Preparation for next cycle begins. Temperature: 15-25°C'),
            ],
          ),
        ),
        actions: [
  ...
  Widget _buildEnhancedLogo(bool isDark) {
    return AnimatedBuilder(
      animation: Listenable.merge([_pulseAnim, _rotateAnim, _glowAnim, _floatAnim]),
          ...
          child: Stack(
            ...
            children: [
                ...
                child: AnimatedBuilder(
                  ...
                  builder: (_, __) => DecoratedBox(
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: Colors.orange.withOpacity(_glowAnim.value * 0.4),
                          blurRadius: 40,
                          spreadRadius: 15,
                        ),
                      ],
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        ...
                        BoxShadow(color: Colors.amber.withOpacity(0.25), blurRadius: 15, spreadRadius: 3),
                      ],
                    ),
                  ),
                ),
              ...
              Container(
                ...
                height: 80,
                decoration: BoxDecoration(
                  gradient: RadialGradient(colors: [Colors.amber[100]!, Colors.orange[300]!]),
                  shape: BoxShape.circle,
                  border: Border.all(color: Colors.white.withOpacity(0.5), width: 2),
                  boxShadow: [BoxShadow(color: Colors.orange.withOpacity(0.3), blurRadius: 20, spreadRadius: 5)],
                ),
                child: const Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      ...
                      Text('ऋतु', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Colors.brown)),
                    ],
                  ),
                ),
              ),
              ...
              ...List.generate(6, (i) {
                ...
                final floatOffset = math.sin(_floatAnim.value * math.pi * 2 + i) * 5;
                return Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      shape: BoxShape.circle,
                      boxShadow: [
                        BoxShadow(color: _ritus[i].colorDark.withOpacity(0.6), blurRadius: 15, spreadRadius: 3),
                      ],
                    ),
                    child: Center(child: Text(_ritus[i].emoji, style: const TextStyle(fontSize: 16))),
                  ),
                );
              }),
            ],
          ...
          builder: (_, __) => ShaderMask(
            shaderCallback: (bounds) => LinearGradient(
              colors: [
                ...
                Colors.yellow.withOpacity(_shimmerAnim.value.clamp(0, 1)),
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ListView.builder(
              scrollDirection: Axis.horizontal,
              itemCount: 6,
              itemBuilder: (_, i) => _buildInteractiveSeasonCard(_ritus[i], isDark, i),
            ),
          ),
        ],
      ),
        ...
        builder: (_, __) => Transform.scale(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [
                  ritu.colorLight.withOpacity(0.4),
                  ritu.colorDark.withOpacity(0.2),
                ],
              ),
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: ritu.colorDark.withOpacity(0.4), width: 2),
              boxShadow: [
                BoxShadow(color: ritu.colorDark.withOpacity(0.3), blurRadius: 12, offset: const Offset(0, 4)),
              ],
            ),
            child: Column(
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: ritu.colorDark.withOpacity(0.2),
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Text(
                    'Tap to learn',
                    style: TextStyle(fontSize: 6, fontWeight: FontWeight.w700, color: ritu.colorDark),
                  ),
                ),
              ],
            ),
          ...
          child: Column(
            children: [
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                children: [
                  _buildMiniFeature('🌾', '6 Seasons', Colors.green),
                  _buildMiniFeature('💧', 'Resource Mgt', Colors.blue),
                  _buildMiniFeature('🏆', '3 Year Goal', Colors.amber),
                  _buildMiniFeature('📚', 'Ancient Wisdom', Colors.purple),
                ],
              ),
            ],
          ),
        ...
        builder: (_, __) => Transform.scale(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [
                  ritu.colorLight.withOpacity(0.3),
                  ritu.colorDark.withOpacity(0.1),
                ],
              ),
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: ritu.colorDark.withOpacity(0.4)),
              boxShadow: [
                BoxShadow(color: ritu.colorDark.withOpacity(0.2), blurRadius: 8),
              ],
            ),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          _buildEnhancedStep('1️⃣', 'SELECT CROP', 'Choose crops that match current season for best growth', Colors.green, isDark),
          _buildEnhancedStep('2️⃣', 'PLANT SEEDS', 'Tap empty plots to plant. Each crop costs 5 seeds', Colors.brown, isDark),
          _buildEnhancedStep('3️⃣', 'CARE DAILY', 'Water and fertilize crops. Manage energy wisely', Colors.blue, isDark),
          _buildEnhancedStep('4️⃣', 'HARVEST', 'Collect crops when golden. Gain seeds and experience', Colors.amber, isDark),
          _buildEnhancedStep('5️⃣', 'SURVIVE', 'Complete 3 years. Adapt to each season\'s challenges', Colors.purple, isDark),
        ],
      ),
    );
  }
  
  Widget _buildEnhancedStep(String num, String title, String desc, Color color, bool isDark) {
    return Padding(
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Center(child: Text(num, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w900))),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(desc, style: TextStyle(fontSize: 10, color: isDark ? Colors.white60 : Colors.black54, height: 1.3)),
                  ],
                ),
              ),
            ],
      ...
      child: Column(
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: [
              _buildWisdomPillar('🌾', 'Sacred Crops', 'Traditional varieties with spiritual significance'),
              _buildWisdomPillar('💧', 'Water Harmony', 'Ancient techniques for water conservation'),
              _buildWisdomPillar('🙏', 'Lunar Farming', 'Following cosmic cycles for planting'),
            ],
          ),
        ],
      ),
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
              colors: [
                ...
                Colors.indigo.withOpacity(0.1 + _shimmerAnim.value * 0.1),
              ],
            ),
            borderRadius: BorderRadius.circular(20),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
                ...
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        Text(ritu.emoji, style: const TextStyle(fontSize: 40)),
                        GestureDetector(
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 14,
                        color: ritu.colorDark.withOpacity(0.7),
                      ),
                    ),
                    Text(
                      ritu.months,
              ...
              // Festivals
              _buildFestivalsSection(ritu.festivals, ritu.colorDark, isDark),
              const SizedBox(height: 20),
              
              // Traditional tips
              _buildTipsSection(ritu.traditionalTips, ritu.colorDark, isDark),
            ],
          ),
        ),
      ),
  ...
  Widget _buildFestivalsSection(List<String> festivals, Color color, bool isDark) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          ...festivals.map((festival) => Padding(
  ...
  Widget _buildCompactSeasonCard(_Ritu ritu, bool isDark) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            ritu.colorLight.withOpacity(0.2),
            ritu.colorDark.withOpacity(0.1),
          ],
        ),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: ritu.colorDark.withOpacity(0.3)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  Widget _tutorialSection(String title, String content, Color color, bool isDark) {
    return Container(
        ...
        children: [
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          gradient: SweepGradient(
                            colors: _ritus.map((r) => r.colorLight).toList()..add(_ritus[0].colorLight),
                          ),
                          boxShadow: [
                            BoxShadow(color: Colors.orange.withOpacity(0.3), blurRadius: 30, spreadRadius: 10),
                          ],
                        ),
                      ),
                    ),
                    ...
                    child: const Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('ऋतु', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Colors.brown)),
                        ],
                      ),
                    ),
                  ),
                    ...
                    return Positioned(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: _ritus[i].colorDark.withOpacity(0.5), blurRadius: 8)],
                        ),
                        child: Center(child: Text(_ritus[i].emoji, style: const TextStyle(fontSize: 14))),
                      ),
                    );
                  }),
                ],
          ...
          ShaderMask(
            shaderCallback: (bounds) => const LinearGradient(
              colors: [Color(0xFFFF6B00), Color(0xFFFFD700), Color(0xFF4CAF50)],
            ).createShader(bounds),
            child: const Text('RITU CHAKRA', style: TextStyle(fontSize: 38, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 4)),
          ),
          const SizedBox(height: 4),
          ShaderMask(
            shaderCallback: (bounds) => const LinearGradient(
              colors: [Color(0xFF8B4513), Color(0xFFDAA520)],
            ).createShader(bounds),
            child: const Text('The Wheel of Seasons', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white, letterSpacing: 2)),
          ),
          const SizedBox(height: 8),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [Colors.green.withOpacity(0.2), Colors.orange.withOpacity(0.1)]),
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: Colors.green.withOpacity(0.3)),
            ),
            child: const Text('🌾 Master Traditional Seasonal Farming 🌾', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Colors.green)),
          ),
          const SizedBox(height: 24),
          ...
          _buildGlassCard([
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                _buildStatBubble('🏆', 'Best', '$_highScore', Colors.amber),
                _buildStatBubble('🪙', 'Coins', '$_coins', Colors.orange),
                _buildStatBubble('📚', 'Wisdom', '$_wisdom', Colors.purple),
              ],
            ),
          ], isDark),
          const SizedBox(height: 16),
          ...
          // Season cards
          const Text('THE 6 RITUS (SEASONS)', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1)),
          const SizedBox(height: 12),
          SizedBox(
            height: 100,
            child: ListView.builder(
          ...
          // Play button
          _buildPremiumButton('🌱  START FARMING', const Color(0xFF4CAF50), _startGame, large: true),
          const SizedBox(height: 16),
          
          // Info card
          _buildGlassCard([
            const Row(children: [Text('📖', style: TextStyle(fontSize: 20)), SizedBox(width: 10), Text('HOW TO PLAY', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1))]),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [ritu.colorLight.withOpacity(0.4), ritu.colorDark.withOpacity(0.2)],
        ),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: ritu.colorDark.withOpacity(0.4)),
        boxShadow: [BoxShadow(color: ritu.colorDark.withOpacity(0.2), blurRadius: 8)],
      ),
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Text(ritu.emoji, style: const TextStyle(fontSize: 24)),
          const SizedBox(height: 4),
          Text(ritu.name, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: ritu.colorDark)),
          Text(ritu.english, style: TextStyle(fontSize: 9, color: isDark ? Colors.white54 : Colors.black38)),
        ],
      ),
    );
  }
  ...
  Widget _buildGameHUD(bool isDark, _Ritu ritu) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [ritu.colorDark.withOpacity(0.8), ritu.colorLight.withOpacity(0.4), Colors.transparent],
          begin: Alignment.topCenter,
          end: Alignment.bottomCenter,
        ),
      ),
      child: Row(
        children: [
          ...
          Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('Year $_year • Day $_dayInRitu/60', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: Colors.white)),
              Text('${ritu.emoji} ${ritu.name}', style: TextStyle(fontSize: 10, color: Colors.white.withOpacity(0.8))),
            ],
          ),
          const Spacer(),
          _buildHudChip('⭐', '$_score', Colors.amber),
          const SizedBox(width: 6),
          _buildHudChip('🌾', '$_harvest', Colors.green),
          const SizedBox(width: 6),
          _buildHudChip('🪙', '$_coins', Colors.orange),
        ],
      ),
    );
  }
  ...
  Widget _buildSeasonBanner(_Ritu ritu, bool isDark) {
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [ritu.colorLight.withOpacity(0.3), ritu.colorDark.withOpacity(0.15)]),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: ritu.colorDark.withOpacity(0.3)),
      ),
      child: Row(
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('${ritu.name} (${ritu.english})', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: ritu.colorDark)),
                Text(ritu.months, style: TextStyle(fontSize: 10, color: isDark ? Colors.white54 : Colors.black38)),
                const SizedBox(height: 4),
                Text(ritu.description, style: TextStyle(fontSize: 10, color: isDark ? Colors.white70 : Colors.black54)),
              ],
            ),
          ),
        ],
  ...
  Widget _buildResourcesBar(bool isDark) {
    return Container(
      padding: const EdgeInsets.all(10),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [(isDark ? Colors.white : Colors.black).withOpacity(0.08), Colors.transparent]),
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: [
          _buildResourceGauge('💧', 'Water', _water, Colors.blue),
          _buildResourceGauge('🌱', 'Seeds', _seeds, Colors.green),
          _buildResourceGauge('💩', 'Fert', _fertilizer, Colors.brown),
          _buildResourceGauge('⚡', 'Energy', _energy, Colors.orange),
        ],
      ),
    );
  }
  
  Widget _buildResourceGauge(String emoji, String label, double value, Color color) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Text(emoji, style: const TextStyle(fontSize: 16)),
        ...
        Text('${value.toInt()}', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: color)),
      ],
    );
  }
  ...
  Widget _buildFarmGrid(bool isDark) {
    ...
    final hasReadyHarvest = _farmGrid.any((row) => row.any((p) => p.isReadyToHarvest));
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: (hasReadyHarvest ? Colors.amber : (hasSelectedCrop ? Colors.green : Colors.blue)).withOpacity(0.15),
          ...
          child: Text(
            ...
            style: TextStyle(
              fontSize: 11,
              fontWeight: FontWeight.w700,
              color: hasReadyHarvest ? Colors.amber[700] : (hasSelectedCrop ? Colors.green : Colors.blue),
            ),
          ),
        ),
        Container(
          padding: const EdgeInsets.all(10),
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: [Colors.brown.withOpacity(0.2), Colors.green.withOpacity(0.1)]),
            borderRadius: BorderRadius.circular(18),
            border: Border.all(color: Colors.brown.withOpacity(0.3)),
            boxShadow: [BoxShadow(color: Colors.brown.withOpacity(0.1), blurRadius: 10)],
          ),
          child: Column(
            children: [
              for (int y = 0; y < 4; y++)
        ...
        Padding(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              _legendItem('🟫', 'Empty', Colors.brown),
              _legendItem('💧', 'Watered', Colors.blue),
              _legendItem('✨', 'Ready!', Colors.amber),
            ],
          ),
        ),
      ],
  ...
  Widget _buildPlot(int x, int y, bool isDark) {
    ...
    Color plotColor = Colors.brown[300]!;
    String displayEmoji = '🟫';
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: RadialGradient(
            colors: [plotColor.withOpacity(0.6), plotColor.withOpacity(0.3)],
          ),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
            color: plot.isReadyToHarvest 
              ? Colors.amber 
              : (plot.isWatered ? Colors.blue : Colors.brown).withOpacity(0.5),
            width: plot.isReadyToHarvest ? 3 : 2,
          ),
          boxShadow: plot.isReadyToHarvest ? [BoxShadow(color: Colors.amber.withOpacity(0.5), blurRadius: 8)] : null,
        ),
        child: Stack(
          alignment: Alignment.center,
          children: [
  ...
  Widget _buildCropSelection(bool isDark) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            const Text('🌱 SELECT CROP', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 0.5)),
            const SizedBox(width: 8),
            Container(
              ...
              child: const Text('⭐ = Best for this season!', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w600, color: Colors.green)),
            ),
          ],
        ),
        const SizedBox(height: 8),
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      colors: isSelected
                        ? [crop.color.withOpacity(0.5), crop.color.withOpacity(0.3)]
                        : [crop.color.withOpacity(0.2), crop.color.withOpacity(0.1)],
                    ),
                    borderRadius: BorderRadius.circular(14),
                    border: Border.all(
                      color: isSelected ? crop.color : crop.color.withOpacity(0.3),
                      width: isSelected ? 2 : 1,
                    ),
                    boxShadow: isSelected ? [BoxShadow(color: crop.color.withOpacity(0.3), blurRadius: 8)] : null,
                  ),
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(crop.emoji, style: const TextStyle(fontSize: 20)),
                      Text(crop.name, style: TextStyle(fontSize: 8, fontWeight: FontWeight.w700, color: crop.color)),
                      if (isIdeal) const Text('⭐', style: TextStyle(fontSize: 8)),
                    ],
                  ),
                ),
              );
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text('🌿 PLOT ACTIONS', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900)),
            const SizedBox(height: 16),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                _buildPlotAction('💧', 'Water', Colors.blue, () { Navigator.pop(context); _waterPlot(x, y); }),
                _buildPlotAction('💩', 'Fertilize', Colors.brown, () { Navigator.pop(context); _fertilizePlot(x, y); }),
              ],
            ),
          ],
        ),
  ...
  Widget _buildPlotAction(String emoji, String label, Color color, VoidCallback onTap) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 16),
  ...
  Widget _buildActionButtons(bool isDark) {
    return Row(
      children: [
        Expanded(child: _buildActionBtn('☀️', 'Next Day', Colors.orange, _advanceDay)),
        const SizedBox(width: 8),
        Expanded(child: _buildActionBtn('😴', 'Rest (+3 days)', Colors.purple, _rest)),
        const SizedBox(width: 8),
        Expanded(child: _buildActionBtn('🛒', 'Shop', Colors.blue, () => _showShop(isDark))),
      ],
    );
  }
  
  Widget _buildActionBtn(String emoji, String label, Color color, VoidCallback onTap) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('🛒 SHOP', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
                Text('🪙 $_coins', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Colors.amber)),
              ],
            ),
            const SizedBox(height: 16),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                _buildShopItem('🌱', 'Seeds', '+20', Colors.green, () => _buyResources('seeds')),
                _buildShopItem('💩', 'Fertilizer', '+15', Colors.brown, () => _buyResources('fertilizer')),
                _buildShopItem('💧', 'Water', '+30', Colors.blue, () => _buyResources('water')),
              ],
            ),
            const SizedBox(height: 10),
            Text('Each item costs 10 coins', style: TextStyle(fontSize: 11, color: isDark ? Colors.white54 : Colors.black38)),
          ],
        ),
      ),
    );
  ...
  Widget _buildShopItem(String emoji, String name, String bonus, Color color, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Text(bonus, style: TextStyle(fontSize: 10, color: color.withOpacity(0.7))),
            const SizedBox(height: 4),
            const Text('🪙 10', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: Colors.amber)),
          ],
        ),
      ),
    );
  ...
  Widget _buildNotifications(bool isDark) {
    return Column(
      children: _notifications.take(3).map((n) => Opacity(
        ...
        child: Container(
          ...
          child: Text(n.message, style: const TextStyle(fontSize: 11, color: Colors.white)),
        ),
      )).toList(),
    );
  }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 16),
            ShaderMask(
              shaderCallback: (bounds) => const LinearGradient(colors: [Colors.amber, Colors.orange]).createShader(bounds),
              child: const Text('MASTER FARMER!', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 3)),
            ),
            const SizedBox(height: 8),
            Text('You mastered the Ritu Chakra in $_year years!', style: TextStyle(color: isDark ? Colors.white60 : Colors.black45)),
            const SizedBox(height: 24),
            
            _buildGlassCard([
              _buildStatRow('⭐', 'Final Score', '$_score', Colors.amber, isDark),
              _buildStatRow('🌾', 'Total Harvest', '$_harvest', Colors.green, isDark),
              _buildStatRow('📚', 'Wisdom Gained', '$_wisdom', Colors.purple, isDark),
              _buildStatRow('🪙', 'Coins Earned', '$_coins', Colors.orange, isDark),
            ], isDark),
            const SizedBox(height: 24),
            
            _buildPremiumButton('🔄 Play Again', Colors.green, _startGame, large: true),
            const SizedBox(height: 12),
  ...
  Widget _buildGlassCard(List<Widget> children, bool isDark) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [(isDark ? Colors.white : Colors.black).withOpacity(0.1), (isDark ? Colors.white : Colors.black).withOpacity(0.03)],
        ),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: (isDark ? Colors.white : Colors.black).withOpacity(0.1)),
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(isDark ? 0.3 : 0.08), blurRadius: 15)],
      ),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          Expanded(child: Text(label, style: TextStyle(fontSize: 13, color: isDark ? Colors.white70 : Colors.black54))),
          Text(value, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: color)),
        ],
      ),
    );
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: color.withOpacity(0.45), blurRadius: large ? 18 : 12, offset: Offset(0, large ? 8 : 5))],
        ),
        child: Center(
          child: Text(label, style: TextStyle(fontSize: large ? 17 : 14, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: large ? 2 : 1)),
        ),
      ),
    );
  }
...
class _Ritu {
  ...
  const _Ritu(this.id, this.name, this.emoji, this.english, this.colorLight, this.colorDark, this.months, this.description, 
    {required this.knowledge, required this.festivals, required this.traditionalTips});
}
...
class _Crop {
  ...
  final Color color;
  const _Crop(this.id, this.emoji, this.name, this.idealSeasons, this.growthDays, this.baseYield, this.color);
}
...
class _WeatherParticle {
  double x, y, vx, vy;
  final String type;
  _WeatherParticle({required this.x, required this.y, required this.vx, required this.vy, required this.type});
}
...
class _SeasonBackgroundPainter extends CustomPainter {
  ...
  _SeasonBackgroundPainter({required this.ritu, required this.progress, required this.isDark});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 15; i++) {
      ...
      paint.color = ritu.colorLight.withOpacity(rand.nextDouble() * 0.1 + 0.03);
      canvas.drawCircle(Offset(x, y), elemSize, paint);
    }
    
    // Ground
    paint
      ..color = Colors.brown.withOpacity(isDark ? 0.15 : 0.1)
      ..style = PaintingStyle.fill;
    
    final groundPath = Path();
    groundPath.moveTo(0, size.height * 0.85);
    for (double x = 0; x <= size.width; x += 30) {
      groundPath.lineTo(x, size.height * 0.85 + math.sin((x + progress * 100) * 0.02) * 10);
    }
  ...
  @override
  bool shouldRepaint(covariant _SeasonBackgroundPainter old) => old.progress != progress || old.ritu.id != ritu.id;
}
