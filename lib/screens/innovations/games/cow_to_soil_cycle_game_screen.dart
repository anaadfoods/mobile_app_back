...
class _GameState extends State<CowToSoilCycleGameScreen>
    ...
    GamePhase.phase1_feeding: {
      ...
      'breeds': [
        ...
        },
        {
          ...
          'origin': 'Punjab',
          'milk': '8-10L/day',
          'special': 'Heat tolerant',
        },
        {
          'name': 'Red Sindhi',
          'origin': 'Sindh',
          'milk': '6-8L/day',
          'special': 'Disease resistant',
        },
        {
          'name': 'Tharparkar',
          'origin': 'Rajasthan',
          'milk': '5-7L/day',
          'special': 'Survives in desert',
        },
      ],
    },
  ...
  void _nextPhase() {
    _playSound('success');
    switch (_phase) {
      case GamePhase.phase1_feeding:
        setState(() => _phase = GamePhase.phase2_collection);
        _startPhase2();
        break;
      case GamePhase.phase2_collection:
        setState(() => _phase = GamePhase.phase3_mixing);
        _startPhase3();
        break;
      case GamePhase.phase3_mixing:
        setState(() => _phase = GamePhase.phase4_application);
        _startPhase4();
        break;
      case GamePhase.phase4_application:
        setState(() => _phase = GamePhase.phase5_defense);
        _startPhase5();
        break;
      case GamePhase.phase5_defense:
        _levelComplete();
        break;
      default:
        break;
  ...
  void _tapFodder(_FodderItem f) {
    ...
    setState(() {
      ...
      if (f.good) {
        _taps++;
        _happiness = (_happiness + 10).clamp(0, 100).toInt();
        _addScore(15);
      } else {
        _happiness = (_happiness - 25).clamp(0, 100).toInt();
        _combo = 0;
        _shake_();
      }
    });
  ...
  void _startPhase3() {
    ...
    _timer = Timer.periodic(const Duration(milliseconds: 80), (t) {
      ...
      if (_ferment >= 1) {
        final ok =
            (_dung - 0.30).abs() < 0.12 &&
            (_urine - 0.20).abs() < 0.10 &&
            (_jaggery - 0.15).abs() < 0.08 &&
            (_water - 0.35).abs() < 0.12;
        if (ok) {
          _addScore(120);
          _showPhaseComplete();
        } else {
        ...
        children: [
          Row(
            children: [
              ...
              _miniStat('⭐', '$_score', AppColors.harvestAmber),
              const SizedBox(width: 6),
              _miniStat('🔥', '${_combo}x', AppColors.rawEarth),
              const SizedBox(width: 6),
              _miniStat('❤️', '$_lives', AppColors.rawEarth),
              const SizedBox(width: 6),
              _miniStat('🎯', 'L$_level', AppColors.deepSoilGreen),
            ],
          ),
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              backgroundColor: AppColors.rawEarth54.withValues(alpha: 0.2),
  ...
  Color _getPhaseColor() => switch (_phase) {
    GamePhase.phase1_feeding => AppColors.deepSoilGreen,
    GamePhase.phase2_collection => AppColors.deepSoilGreen,
    GamePhase.phase3_mixing => AppColors.harvestAmber,
    GamePhase.phase4_application => AppColors.rawEarth,
    GamePhase.phase5_defense => AppColors.rawEarth,
    _ => AppColors.deepSoilGreen,
  };
  ...
  Widget _menuUI(bool isDark) {
    final primaryColor = AppColors.parchment; // Forest green
    final bgColor = isDark ? AppColors.parchment : AppColors.parchment;
  ...
  Widget _buildCleanPhaseSection(
    bool isDark,
    Color primary,
    Color textColor,
    Color subtleColor,
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  _buildKnowledgeItem(
                    '🐄 Desi Cow Breeds',
                    'Gir, Sahiwal, Tharparkar - native breeds perfectly adapted to Indian climate and produce nutrient-rich A2 milk.',
                  ),
                  const SizedBox(height: 12),
                  _buildKnowledgeItem(
                    '🧪 Jeevamrut Formula',
                    'Cow dung + urine + jaggery + pulse flour + water = Natural fertilizer with 9x more microbes than chemicals.',
                  ),
                  const SizedBox(height: 12),
                  _buildKnowledgeItem(
                    '🌍 Soil Health',
                    'One cow can enrich 1 acre of land naturally, reducing need for chemical fertilizers by 80%.',
                  ),
                  const SizedBox(height: 12),
                  _buildKnowledgeItem(
                    '🌾 Crop Protection',
                    'Natural pesticides from cow products protect crops without harming beneficial insects or soil life.',
                  ),
                ],
              ),
            ),
              ...
              child: Stack(
                ...
                children: [
                  ...
                  ...List.generate(5, (i) {
                    ...
                    final floatOffset =
                        math.sin(_float.value * math.pi * 2 + i) * 5;
                    final phaseIcons = ['🐄', '🪣', '🧫', '🌱', '🛡️'];
                    final phaseColors = [
                      AppColors.deepSoilGreen,
                      AppColors.deepSoilGreen,
                    ...
                    return Positioned(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(
                              color: phaseColors[i].withValues(alpha: 0.6),
                              blurRadius: 15,
                              spreadRadius: 3,
                            ),
                          ],
                ...
                shaderCallback:
                    (bounds) => LinearGradient(
                      colors: [
                        ...
                        AppColors.deepSoilGreen.withValues(
                          alpha: _shimmer.value.clamp(0, 1),
                        ),
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      _buildMiniFeature('🌱', 'Organic', AppColors.rawEarth),
                      _buildMiniFeature(
                        '📚',
                        'Educational',
                        AppColors.harvestAmber,
              ...
              child: Column(
                children: [
                  ...
                  if (phaseIndex == 0 && edu.containsKey('breeds'))
                    _buildCowBreedsSection(
                      edu['breeds'] as List<Map<String, String>>,
                      isDark,
                    ),
          ...
          ...breeds.map(
            (breed) => Padding(
              ...
              child: Container(
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(
                  color: AppColors.deepSoilGreen.withValues(alpha: 0.05),
                  borderRadius: BorderRadius.circular(12),
                ),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildCompactPhaseCard(String title, int index, bool isDark) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            phaseColors[index].withValues(alpha: 0.2),
            phaseColors[index].withValues(alpha: 0.1),
          ],
        ),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: phaseColors[index].withValues(alpha: 0.3)),
      ),
      child: Row(
        children: [
        ...
        Positioned(
          ...
          child: Column(
            children: [
                    ...
                    Expanded(
                      child: ClipRRect(
                        ...
                        child: LinearProgressIndicator(
                          ...
                          backgroundColor: AppColors.rawEarth54.withValues(
                            alpha: 0.2,
                          ),
              ...
              Text(
                'Tap healthy fodder! Avoid chemicals!',
                style: TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.w600,
  ...
  Widget _phase2UI(bool isDark, Size size) {
    return GestureDetector(
      onHorizontalDragUpdate:
          (d) => setState(
            () =>
                _bucketX = (_bucketX + d.delta.dx / size.width).clamp(0.1, 0.9),
          ),
      child: Stack(
        children: [
          Positioned(
            ...
            child: Center(
              child: Column(
                children: [
                  ...
                  Text(
                    'Collected: $_collected / $_collectTarget',
                    style: TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.w800,
                      color:
          ...
          ..._drops.map(
            (d) => Positioned(
              ...
              child: Container(
                ...
                height: 44,
                decoration: BoxDecoration(
                  color: (d.good ? AppColors.harvestAmber : AppColors.rawEarth)
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: AppColors.rawEarth, width: 3),
                    boxShadow: [
                      BoxShadow(
                        color: AppColors.rawEarth.withValues(alpha: 0.4),
                        blurRadius: 12,
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Text(
              'Target: Dung ~30% | Urine ~20% | Jaggery ~15% | Water ~35%',
              style: TextStyle(
                fontSize: 11,
                fontWeight: FontWeight.w700,
                color: AppColors.harvestAmber,
          ...
          Container(
            height: 160,
            decoration: BoxDecoration(
              color: AppColors.rawEarth.withValues(alpha: 0.15),
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
                color: AppColors.rawEarth.withValues(alpha: 0.4),
                width: 3,
  ...
  Widget _ingredientRow(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(14),
        border: Border.all(
          color: close ? AppColors.deepSoilGreen : c.withValues(alpha: 0.3),
          width: close ? 2 : 1,
        ),
      ),
      child: Row(
        children: [
          ...
          Expanded(
            child: SliderTheme(
              ...
              child: Slider(
                value: val,
                onChanged: _fermenting ? null : (v) => _setIngredient(type, v),
              ),
            ),
          ),
          GestureDetector(
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: 12,
              fontWeight: FontWeight.w900,
              color: close ? AppColors.deepSoilGreen : c,
            ),
          ),
        ],
      ),
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 6),
          Text(
            'Tap to place nutrients. Roots will grow toward them!',
            style: TextStyle(
              fontSize: 12,
              color: isDark ? AppColors.parchment60 : AppColors.charcoal45,
            ),
          ...
          Container(
            ...
            child: Text(
              'Placed: $_nutrients / $_nutrientTarget',
              style: const TextStyle(
                fontSize: 14,
                fontWeight: FontWeight.w800,
                color: AppColors.deepSoilGreen,
          ...
          Expanded(
            child: Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(
                color: AppColors.rawEarth.withValues(alpha: 0.12),
                borderRadius: BorderRadius.circular(20),
                border: Border.all(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          ...
                          width: cell.root || cell.nutrient ? 2 : 1,
                        ),
                      ),
                      child: Center(
                        child: Text(
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              _legend('🌿', 'Root', AppColors.deepSoilGreen),
              const SizedBox(width: 20),
              _legend('✨', 'Nutrient', AppColors.harvestAmber),
              const SizedBox(width: 20),
              _legend('⬜', 'Soil', AppColors.rawEarth),
            ],
          ),
        ],
      ),
  ...
  Widget _legend(String e, String l, Color c) => Row(
    children: [
      Text(e),
    ...
    return GestureDetector(
      onTapDown:
          (d) => _deployDefender(
            ...
            (d.localPosition.dy - 50) / (size.height * 0.7),
          ),
      child: Stack(
        children: [
          // Timer display
          ...
          ..._pests.map(
            (p) => Positioned(
              ...
              child: Container(
                ...
                height: 44,
                decoration: BoxDecoration(
                  color: AppColors.rawEarth.withValues(alpha: 0.25),
                  shape: BoxShape.circle,
                  border: Border.all(color: AppColors.rawEarth, width: 2),
                ),
                child: Center(
          ...
          Positioned(
            ...
            child: Center(
              child: Container(
                ...
                child: Text(
                  '👆 TAP anywhere to deploy defenders! (${_defenders.length}/${4 + _level})',
                  style: TextStyle(
                    fontSize: 13,
                    fontWeight: FontWeight.w800,
                    color: isDark ? AppColors.parchment : AppColors.charcoal87,
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          _endStat('Final Score', '$_score', AppColors.harvestAmber, isDark),
          const SizedBox(height: 10),
          _endStat(
            'Max Combo',
            '${_maxCombo}x',
          ...
          const SizedBox(height: 10),
          _endStat('Level', '$_level', AppColors.deepSoilGreen, isDark),
          const SizedBox(height: 28),
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              _btn('🔄 Retry', AppColors.deepSoilGreen, _startGame),
    ...
    child: SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'You completed the entire Cow → Soil Cycle! 🐄→🌱→🥬',
            style: TextStyle(
              fontSize: 14,
              fontWeight: FontWeight.w600,
              color: AppColors.deepSoilGreen,
          ...
          const SizedBox(height: 20),
          _endStat('Score', '$_score', AppColors.harvestAmber, isDark),
          const SizedBox(height: 10),
          _endStat('Lives', '$_lives', AppColors.rawEarth, isDark),
          const SizedBox(height: 10),
          _endStat(
            'Max Combo',
            '${_maxCombo}x',
...
class _Cell {
  final int x, y;
  bool nutrient = false, root = false;
  _Cell(this.x, this.y);
}
